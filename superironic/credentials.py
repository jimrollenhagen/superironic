...
def pull_env_credential(env, param, value):
    """
    Dissects a keyring credential lookup string from the superironic config file
...
def password_get(username=None):
    ...
    """
    try:
        return keyring.get_password('superironic', username).encode('ascii')
    except:
        return False
...
def password_set(username=None, password=None):
    ...
    """
    try:
        keyring.set_password('superironic', username, password)
        return True
    except:
        return False
